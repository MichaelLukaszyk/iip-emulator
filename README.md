...
1. Update parallel.sbatch with your credentials.
2. Ensure there is a conda environment titled 'tardis' with TARDIS installed.
3. Create run/grid.csv using run/make_grid.ipynb
4. Ensure tardis_data/base_config.yml is correct.
5. If modifying shell structure, update make_csvy.py
6. Run 'bash run/submit.sh'
